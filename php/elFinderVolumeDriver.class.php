...
abstract class elFinderVolumeDriver {
	...
	protected $options = array(
		...
		'xsendfilePath'=> '',
        // 文件夹压缩包下载所有文件大小限制，单位为M
        'compressionLimit' => 500,
	);
	...
	public function zipdl($hashes) {
		...
		if ($dir = $this->getItemsInHand($hashes)) {
            ...
            //获取文件或文件夹所有文件的大小
            $size = 0; $cl = $this->options['compressionLimit'] * 1024 * 1024;
            foreach($files as $file) {
                ...
                if ($size > $cl){
                    return $this->setError('下载文件大小限制' . $this->options['compressionLimit'] . 'M内');
                }
            }
